...
class SampleRate():
    ...
    supported_rates = [ (20,'kHz'), ( 25, 'kHz'), ( 50, 'kHz'), ( 100, 'kHz'), ( 200, 'kHz'), (250, 'kHz'), 
                        ( 500, 'kHz'), ( 1, 'MHz'), ( 2, 'MHz'), ( 5, 'MHz'), ( 10, 'MHz'), ( 20, 'MHz'), 
    ...
    def value(self):
        ...
        return self.rate * (1e+3 if self.unit == 'kHz' else 1e+6 if self.unit == 'MHz' else unsupported_unit(self.unit))
...
class Hantek():
    ...
    def __init__(self, device=DEVICE, sample_rate=SAMPLE_RATE, number_of_channels=NUMBER_OF_CHANNELS,
    ...
    def sigrok_command(self):
        return (f'sigrok-cli -d {self.device} --config "samplerate={self.sample_rate.cli_rate}:'
                f'buffersize={self.buffer_size}" -O analog --continuous')

    def open(self):
        try:
            self._process = subprocess.Popen(self.sigrok_command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            self._opened=True
        except:
            self._opened=False
    ...
    def acquire(self, number_of_samples):
        ...
        if self._opened:
            self.samples = numpy.zeros(shape=(self.number_of_channels, number_of_samples))
